...
module.exports = {
  name: "queue",
  description: "Xem danh sách phát hiện tại (nếu có).",
...
${await Promise.all(current.map(data =>
          `\n\`${sayı++}\` | [${data.title}](${data.url}) | **${data.author}** (Thêm bởi: <@${data.requestedBy.id}>)`
        ))}`)
    ...
    await interaction.reply({
      ...
      collector.on("collect", async (button) => {
        if (button.customId === "close") {
          ...
          return button.reply({ content: `✅ Lệnh đã bị huỷ!`, ephemeral: true }).catch(e => { })
        } else {
      ...
      collector.on("end", async (button) => {
        ...
        const embed = new EmbedBuilder()
          .setTitle(`Danh sách phát hiện tại - Time Ended!`)
          //.setThumbnail(interaction.guild.iconURL({ size: 2048, dynamic: true }))
          .setColor('007fff')
          .setDescription(`Lệnh đã hét hiệu lực, gõ \`/queue\` để dùng lại.`)
